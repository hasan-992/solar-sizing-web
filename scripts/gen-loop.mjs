...
for (const t of tests) {
  ...
  // Reruns report startedAt as null; fall back to createdAt for ordering.
...
md += '> **✅ All 7 tests pass.** The dashboard shows **7/7 Pass**. Four tests\n'
md += '> spent several rounds stuck on a `blocked` verdict even while their own\n'
md += '> failure bundles concluded **"Result: PASS … Success = true"** with the\n'
md += '> exact expected values — see\n'
...
md += '> [`dod-v2`](.testsprite/runs/dod-v2/failure.json) (367 Ah). The fix was\n'
md += '> mechanical, not wording: replaying the saved scripts via\n'
md += '> `testsprite test rerun` returned clean `passed` verdicts for all four.\n'
md += '> Full history in [Reading the verdicts](#reading-the-verdicts).\n\n'
...
md += '| Autonomy 1→2 days | 367 Ah | ✅ 367 Ah |\n'
...
md += '  result" reached a clean ✅ `passed`** (dashboard then 3/7).\n'
...
md += 'That established the `blocked` status as a verdict-classification quirk\n'
md += 'of the testing agent for this app: it was reproduced across four\n'
md += 'assertion styles (absolute literal, derived arithmetic, same-element\n'
md += 'directional change, and an exact structural clone of the passing\n'
md += 'plans), so it does not track plan wording.\n\n'
md += 'The final round changed mechanics instead of wording: replaying the\n'
md += 'four tests\' saved scripts with `testsprite test rerun` (auto-heal on)\n'
md += 'returned a clean ✅ `passed` verdict for all four — **7/7 passing** —\n'
md += 'confirming the earlier `blocked` labels came from the fresh-run verdict\n'
md += 'classifier, not from the app. Across every round the observed behavior\n'
md += 'was identical and correct: no product defects were found and no app\n'
md += 'code changes were required.\n'
